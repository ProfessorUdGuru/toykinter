...
Along with some configurable widgets to keep ttk and Windows themes from altering your design intention, this demo repo includes a system for reconfiguring the app's fonts and colors globally and instantly with the push of a button. The system detects subclasses, so to use this system, all widgets are inherited from Tkinter widgets. If you try to use tkinter.Label, for example, you'll get an error because it doesn't have a subclass detection system built into it for the reconfiguration. Most of the inherited widgets are in widgets.py. There you will see that a method widget.winfo_subclass() is built into every widget class with something like toykinter's Labelx, and then the configuration system is inheriting from Labelx to create the widgets that will actually work with the system. 
...
\u2022 The Colors tab needs a little formatting.
